...
def remove_work_item_link(
    ...
    """Remove a relation from a work item by its index.

    Args:
        work_item_api_call: Work-item-level ADO API call (from
            get_work_item_api_call).
        relation_index: Zero-based index of the relation in the work item's
            ``relations`` list.
    ...
    return patch_work_item(
        work_item_api_call,
        [JsonPatchRemove(path=f"/relations/{relation_index}")],
    )
...
def iter_work_item_details(
    ...
    for batch_start in range(0, len(work_item_id_list), _WORK_ITEM_BATCH_SIZE):
        ...
        if work_item_field_list:
            request = WorkItemBatchRequest(ids=batch, fields=work_item_field_list)
        else:
            request = WorkItemBatchRequest(ids=batch, expand=WorkItemExpand.RELATIONS)
...
def create_work_item(
    ...
    *,
    ...
    json_patch_list: list[JsonPatchAdd | JsonPatchRemove] = [
    ...
    for field_name, fmt in (multiline_fields_format or {}).items():
        json_patch_list.append(
            JsonPatchAdd(path=f"/multilineFieldsFormat/{field_name}", value=fmt)
        )
    json_patch_list.extend(
        JsonPatchAdd(
            path="/relations/-",
            value=link.model_dump(mode="json", exclude_defaults=True),
        )
...
def sync_work_item_tags(
    work_item_api_call: ApiCall,
    desired: set[str],
) -> None:
    """Synchronise work item tags to match *desired* exactly in one API call.

    Fetches the current tags once, and — if the set differs — issues a single
    patch to replace the tag field with the desired set.  Does nothing when the
    current tags already match.

    Args:
        work_item_api_call: Work-item-level ADO API call (from
            get_work_item_api_call).
        desired: The exact set of tag names the work item should have after
            the call.
    """
    current = set(get_work_item_tags(work_item_api_call))
    if current == desired:
        return
    patch_work_item(
        work_item_api_call,
        [
            JsonPatchAdd(
                path="/fields/System.Tags",
                value=_format_work_item_tags(list(desired)),
            )
        ],
    )
