...
class DistributedTaskSession(BuildTask):
    ...
    **Why it exists:** ``DistributedTaskSession`` bundles the five UUIDs and
    the owning :class:`~pyado.oop.pipelines.build.Build` together so the
    caller never constructs raw ``ApiCall`` URLs manually.  By extending
    :class:`~pyado.oop.pipelines.build_timeline.BuildTask` it also exposes
    the same read properties (``name``, ``state``, ``result``, ``log``,
    ``issues``) that a normal task has, with lazy loading from the timeline on
    first access — useful for polling an external task's own record to detect
    ...
    :class:`~pyado.oop.pipelines.build_timeline.BuildTask` (loaded lazily on
    first access).  Exposes the write operations (feed messages, log entries,
    timeline issue updates, task completion) that require the distributed-task
    runtime variables only available during pipeline execution.

    Instances are obtained from :meth:`Build.get_distributed_task_session`.
    ...
    Attributes:
        _build: The Build this session belongs to.
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            build: The Build that owns this session.
    ...
    def project(self) -> "Project":
        """Project this session belongs to — zero-cost."""
        return self._build.project

    @property
    def org(self) -> "Organization":
        """Organisation this session belongs to — zero-cost."""
