...
class WorkItemTypeName(StrEnum):
    ...
    Example::
        ...
        create_work_item(api, {
            WorkItemFieldName.WORK_ITEM_TYPE: WorkItemTypeName.BUG,
...
class WorkItemRelation(AdoBaseModel):
...
class WorkItemInfo(AdoBaseModel):
...
class _WorkItemInfoResults(AdoBaseModel):
...
class SprintIterationAttributes(AdoBaseModel):
...
class SprintIterationInfo(AdoBaseModel):
...
class _SprintIterationInfoResults(AdoBaseModel):
    count: int
    value: list[SprintIterationInfo]


class WorkItemRef(AdoBaseModel):
...
class _WorkItemCommentResults(AdoBaseModel):
...
class WorkItemAttachmentRef(AdoBaseModel):
    """A reference to a file attachment uploaded to ADO."""

    id: WorkItemAttachmentId
    url: AnyUrl
...
class WorkItemBatchRequest(AdoBaseModel):
    ...
    expand: WorkItemExpand | None = Field(
        default=None, validation_alias="$expand", serialization_alias="$expand"
    )
...
class ClassificationNodeAttributes(AdoBaseModel):
...
class ClassificationNode(AdoBaseModel):
    ...
    name: str
    path: str | None = None
    structure_type: ClassificationNodeType | None = None
...
class TeamFieldValue(AdoBaseModel):
...
class _WorkItemCommentRequest(AdoBaseModel):
    """Internal: request body for adding a work item comment."""

    text: str


class ClassificationNodeRequest(AdoBaseModel):
...
class ClassificationNodePatchRequest(AdoBaseModel):
...
class _TeamIterationRef(AdoBaseModel):
...
def get_work_item_attachment_bytes(
    project_api_call: ApiCall,
    attachment_id: WorkItemAttachmentId,
) -> bytes:
...
class WorkItemQuery(AdoBaseModel):
    ...
    children: list["WorkItemQuery"] = Field(default_factory=list)
    wiql: str | None = None
    query_type: WorkItemQueryType | None = None
...
class WorkItemTypeIcon(AdoBaseModel):
...
class WorkItemTypeInfo(AdoBaseModel):
    ...
    description: str = ""
    color: str | None = None
    icon: WorkItemTypeIcon | None = None
    is_disabled: bool = False


class WorkItemStateCategory(StrEnum):
    """State category values for work item type states.

    ADO groups work item states into five fixed categories regardless of
    the process template or custom state names.  The category controls
    how the state appears in boards, backlogs, and analytics.
    """
...
class WorkItemFieldType(StrEnum):
    """Field type values returned by the WIT fields API.

    These are the primitive storage types for work item fields as defined
    by Azure DevOps.  They appear in ``WorkItemFieldInfo.field_type`` and
    ``ProjectFieldInfo.field_type``.
    """
...
class WorkItemStateInfo(AdoBaseModel):
...
class WorkItemFieldInfo(AdoBaseModel):
...
class WorkItemTypeCategoryInfo(AdoBaseModel):
    """A work item type category (e.g. ``Requirements``, ``Tasks``)."""

    name: str
    reference_name: str
    default_work_item_type: WorkItemTypeInfo | None = None
    work_item_types: list[WorkItemTypeInfo] = Field(default_factory=list)


class _WorkItemTypeResults(AdoBaseModel):
    """Internal: container for work item type list results."""

    value: list[WorkItemTypeInfo]


class _WITStateResults(AdoBaseModel):
    """Internal: container for WIT state list results."""

    value: list[WorkItemStateInfo]


class _WITFieldResults(AdoBaseModel):
    """Internal: container for WIT field list results."""

    value: list[WorkItemFieldInfo]


class _WorkItemTypeCategoryResults(AdoBaseModel):
    """Internal: container for work item type category list results."""

    value: list[WorkItemTypeCategoryInfo]


def iter_work_item_types(
    project_api_call: ApiCall,
    ...
    """
    result = project_api_call.get("wit", "workitemtypes", version=_WIT_API_VERSION)
    yield from _WorkItemTypeResults.model_validate(result).value
...
def list_work_item_types(
    ...
    return list(iter_work_item_types(project_api_call))


def iter_work_item_type_states(
    project_api_call: ApiCall,
    work_item_type: str,
    ...
    Args:
        project_api_call: Project-level ADO API call.
        work_item_type: Work item type reference name (e.g. ``"Bug"``).
    ...
    result = project_api_call.get(
        ...
        work_item_type,
        "states",
        version=_WIT_API_VERSION,
    )
    yield from _WITStateResults.model_validate(result).value


def list_work_item_type_states(
    project_api_call: ApiCall,
    work_item_type: str,
) -> list[WorkItemStateInfo]:
    """Return the state definitions for a specific work item type as a list."""
    return list(iter_work_item_type_states(project_api_call, work_item_type))


def iter_work_item_type_fields(
    project_api_call: ApiCall,
    work_item_type: str,
    ...
    yield from _WITFieldResults.model_validate(result).value


def list_work_item_type_fields(
    project_api_call: ApiCall,
    work_item_type: str,
) -> list[WorkItemFieldInfo]:
    """Return the field definitions for a specific work item type as a list."""
    return list(iter_work_item_type_fields(project_api_call, work_item_type))


def iter_work_item_type_categories(
    project_api_call: ApiCall,
    ...
    """
    result = project_api_call.get(
        "wit", "workitemtypecategories", version=_WIT_API_VERSION
    )
    yield from _WorkItemTypeCategoryResults.model_validate(result).value


def list_work_item_type_categories(
    project_api_call: ApiCall,
) -> list[WorkItemTypeCategoryInfo]:
    """Return all work item type categories in a project as a list."""
    return list(iter_work_item_type_categories(project_api_call))
